...
public class DrawableBox extends BaseDrawable implements IDrawable {
	...
	public void readFrom(DataInputStream stream, Short changeMask) {
		try {
			...
			if (ByteUtils.get(changeMask, OPACITY_CHANGED)) opacity = stream.readFloat();

			if (ByteUtils.get(changeMask, COLOR2_CHANGED)) color2 = stream.readInt();

			if (ByteUtils.get(changeMask, OPACITY2_CHANGED)) opacity2 = stream.readFloat();
	...
	public void writeTo(DataOutputStream stream, Short changeMask) {
		try {
			...
			if (ByteUtils.get(changeMask, Y_CHANGED)) stream.writeShort(y);

			if (ByteUtils.get(changeMask, WIDTH_CHANGED)) stream.writeShort(width);

			if (ByteUtils.get(changeMask, HEIGHT_CHANGED)) stream.writeShort(height);

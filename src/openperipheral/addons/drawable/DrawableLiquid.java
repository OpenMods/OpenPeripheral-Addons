...
public class DrawableLiquid extends BaseDrawable {
	...
	public DrawableLiquid(ISurface surface, int x, int y, int width,
			int height, int id) {
		super(surface);
		this.x = (short)x;
		this.y = (short)y;
		...
		if (fluidIcon != null && fluidIcon.getIconHeight() > 0 && fluidIcon.getIconWidth() > 0) {
			...
			for (float xIteration = 0; xIteration < xIterations; xIteration++) {
				for (float yIteration = 0; yIteration < yIterations; yIteration++) {
					...
					GlassesRenderingUtils.drawTexturedQuadAdvanced(x + xIteration * fluidIcon.getIconWidth(), y + yIteration * fluidIcon.getIconHeight(), fluidIcon, xDrawSize * fluidIcon.getIconWidth(), yDrawSize
							* fluidIcon.getIconHeight(), xDrawSize, yDrawSize, this.alpha);
				}
			}
		}
	}
